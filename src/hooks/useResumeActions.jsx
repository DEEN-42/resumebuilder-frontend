...
export const useResumeActions = (
  ...
  const handleCreateResume = useCallback(async (formData) => {
    if (formData.title && formData.description && formData.template) {
      try {
        ...
        const response = await fetch('http://localhost:3030/resumes/create', {
          method: 'POST',
          headers: {
  ...
  const handleDeleteResume = useCallback(async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3030/resumes/delete/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleShareResume = useCallback(async (id, shareEmail) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3030/resumes/share/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
